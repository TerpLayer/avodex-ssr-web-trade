...
enum TrailingModeEnum {
  turnRate = "turnRate", //回调幅度
  priceDiff = "priceDiff", //价距
}
...
const Main: React.FC<Props> = ({ className, tradeSide, onSuccess }) => {
  ...
  const { tradeRecentOnce, orderConfirm_trailingStop } = store.trade;
  ...
  const [trailingMode, setTrailingMode] = useState<TrailingModeEnum>(TrailingModeEnum.turnRate);
  const [trailingValue, setTrailingValue] = useState("");
  const [isErrTrailingValue, setIsErrTrailingValue] = useState(false);
  const [isActivePrice, setIsActivePrice] = useState(false);
  const [activePrice, setActivePrice] = useState("");

  const isTurnRate = useMemo(() => trailingMode === TrailingModeEnum.turnRate, [trailingMode]);
  const trailingModeLabelObj = useMemo(() => {
    return {
      [TrailingModeEnum.turnRate]: t("trade.proportion"),
      [TrailingModeEnum.priceDiff]: t("trade.priceDiff"),
    };
  }, []);
  const trailingModeItems: MenuProps["items"] = useMemo(() => {
    return [
      {
        key: TrailingModeEnum.turnRate,
        label: <a onClick={() => setTrailingMode(TrailingModeEnum.turnRate)}>{trailingModeLabelObj[TrailingModeEnum.turnRate]}</a>,
      },
      {
        key: TrailingModeEnum.priceDiff,
        label: <a onClick={() => setTrailingMode(TrailingModeEnum.priceDiff)}>{trailingModeLabelObj[TrailingModeEnum.priceDiff]}</a>,
      },
    ];
  }, [trailingModeLabelObj]);

  useEffect(() => {
    setTrailingValue("");
  }, [trailingMode]);
  useEffect(() => {
    setIsErrTrailingValue(false);
  }, [trailingValue]);
  useEffect(() => {
    setActivePrice("");
  }, [isActivePrice]);
  ...
  const canBuyOrSell = useMemo(() => {
    const unit = " " + currencyGet.toUpperCaseCurrency();
  ...
  const handleBlurTurnRate = useCallback(() => {
    if (trailingValue && +trailingValue < 0.1) setTrailingValue("0.1");
  }, [trailingValue]);
  const handleBlurPriceDiff = useCallback(() => {
    if (!tradeRecentOnce || !trailingValue) return;
    if (+trailingValue > +tradeRecentOnce.p) {
      setTrailingValue(tradeRecentOnce.p);
    }
  }, [trailingValue, tradeRecentOnce]);
  ...
  const apiResPostOrder = useCallback(() => {
    ...
    const data = {
      ...
      [isTurnRate ? "turnRate" : "priceDiff"]: trailingValue,
    };

    activePrice && (data.activePrice = activePrice);
  ...
  }, [loading, name, tradeSide, type, inputValueConvert, isTurnRate, trailingValue, activePrice]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (!+trailingValue) return setIsErrTrailingValue(true);
    if (!+inputValue) return setIsErrInputValue(true);

    checkRiskTip(start);

    function start() {
      if (!orderConfirm_trailingStop) return todo();

      let checked = !orderConfirm_trailingStop;
      ...
      ModalAlert({
        ...
        onOk: (close) => {
          close();
          store.trade.updateState({ orderConfirm_trailingStop: !checked });
          Storage.set("orderConfirm_trailingStop", !checked);
          todo();
        },
          ...
          <div className={styles.orderConfirm}>
            ...
            <div>
              ...
              <div>
                ...
                <div>{isTurnRate ? trailingValue + "%" : trailingValue + " " + coinPriceUpperCase}</div>
              </div>
              ...
              {!!activePrice && (
                <div>
                  ...
                  <div>{activePrice + " " + coinPriceUpperCase}</div>
                </div>
              )}
              <div>
    ...
    <div className={cx(styles.main, className)}>
      ...
      <div className={styles.content}>
        <Dropdown
          ...
          menu={{
            items: trailingModeItems,
            selectable: true,
            selectedKeys: [trailingMode],
          }}
        >
          <button className={cx("btnTxt btnDrop", styles.widget, styles.dropdownTrigger)} onClick={(e) => e.preventDefault()}>
            {trailingModeLabelObj[trailingMode]}
          </button>
        </Dropdown>

        {trailingMode === TrailingModeEnum.turnRate && (
          <div className={cx(styles.turnRate)}>
            <AppInputNumber
              className={styles.widget}
              prefix={
                <Tooltip placement="top" title={t("trade.turnRateTip")}>
                  <span className={cx(styles.turnRateLab)}>{t("trade.turnRate")}</span>
                </Tooltip>
              }
              suffix={"%"}
              value={trailingValue}
              onInput={setTrailingValue}
              disabled={loading}
              isErr={isErrTrailingValue}
              point={1}
              // min={0.1}
              onBlur={handleBlurTurnRate}
              max={99.9}
              noBtns={true}
              isStepPoint={true}
            />

            <button className={cx("btnTxt", styles.widget)} onClick={() => setTrailingValue("5")}>
              5%
            </button>
            <button className={cx("btnTxt", styles.widget)} onClick={() => setTrailingValue("10")}>
              10%
            </button>
          </div>
        )}
        {trailingMode === TrailingModeEnum.priceDiff && (
          <AppInputNumber
        ...
        <div className={styles.activePrice}>
          <div>
            <Checkbox className={styles.activePriceCheckbox} checked={isActivePrice} onChange={(e) => setIsActivePrice(e.target.checked)}>
              <Tooltip placement="top" title={t("trade.activePriceTip")}>
                <span className={cx(styles.activePriceLab)}>{t("trade.activePrice")}</span>
              </Tooltip>
            </Checkbox>
          </div>
          ...
          {isActivePrice && (
            <AppInputNumber
              className={styles.input}
              prefix={t("trade.activePrice")}
              suffix={coinPriceUpperCase}
              value={activePrice}
              onInput={setActivePrice}
              disabled={loading}
              point={coinPricePrecisionMarket}
              isStepPoint={true}
            />
