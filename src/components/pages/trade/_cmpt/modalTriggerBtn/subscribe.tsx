...
const Subscribe: React.FC<Props> = ({ className, children, isIcon, currency }) => {
  ...
  const handleClick = useCallback(() => {
    ...
    store.trade.updateState({
      modalSubscribeRedeem: {
        open: true,
        modalType: AppModalSubscribeRedeemModalTypeEnum.subscribe,
        currency,
  ...
  return (
    <button className={cx("btnTxt", className)} onClick={handleClick}>
      {children || (isIcon ? <AzSvg icon={"add"} /> : t("trade.subscribe"))}
    </button>
  );
};
